...
let allDataQuery = `SELECT e.id, e.first_name, e.last_name, e.manager_id, r.title, r.salary, d.department_name, CONCAT(m.first_name, " ", m.last_name) "manager"
  FROM employee e
...
class DB {
    ...
    this.connection.query("SELECT id, title FROM role_table", (error, results) => {
      ...
      inquirer
        ...
        .then((answer) => {
          ...
          const sql = mysql.format(allDataQuery + "WHERE ?", { role_id: roleId });
          this.connection.query(sql, (error, results) => {
            if (error) return error;
            if (results.length < 1) {
              return callback(`\nNo employees found with role '${roleName}'\n`);
            } else {
              return callback(results);
            }
          });
  ...
  addEmployee2 = function (callback) {
    inquirer
      .prompt([
        {
          type: "input",
