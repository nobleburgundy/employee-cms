...
class DB {
  ...
  removeEmp2 = (cb) => {
    this.connection.query("SELECT first_name, last_name FROM employee", (error, results) => {
        ...
        .prompt([
          {
            ...
            choices: function () {
              let choiceArray = [];
              results.forEach((e) => choiceArray.push(e.first_name + " " + e.last_name));
              return choiceArray;
            },
          },
        ])
        .then((answer) => {
          const first = answer.empToRemove.substr(0, answer.empToRemove.indexOf(" ")).trim();
          const last = answer.empToRemove.substr(answer.empToRemove.indexOf(" ")).trim();
          this.removeEmployee(first, last, (res) => {
            return cb(res);
          });
        });
    });
